...
> [!note]
> **Tradeoff:** A singleton set {x} always works (K(S) ≈ K(x)), but fails the plausibility condition. The "all strings" set always satisfies the plausibility condition (K(x|S) ≈ log |S|), but K(S) is tiny. Sophistication picks the sweet spot in between: the smallest model that's still large enough to treat x as random.
...
In deterministic systems:
  ...
  As for plausibility:
  - For a deterministic system, theres only one possible S(t) thus the RHS is $\le$ 0, which our RHS is clearly $\ge$ since it is lower bounded by 0.
